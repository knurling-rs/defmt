//! `defmt` global logger over stdout
//!
//! WARNING using the `println!` macro or `Stdout` API will corrupt
...
struct SemihostingEncoder {
    ...
    handle: UnsafeCell<Option<std::io::Stdout>>,
}
...
impl SemihostingEncoder {
    ...
    fn acquire(&self) {
        ...
        unsafe {
            ...
            let handle: &mut Option<std::io::Stdout> = &mut *self.handle.get();
            if handle.is_none() {
                *handle = Some(std::io::stdout());
            }
